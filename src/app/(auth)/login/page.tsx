  ...
  return (
    <div className="flex justify-center items-center min-h-[70vh]">
      <div className="w-full max-w-md bg-white p-8 rounded-lg shadow-sm border border-gray-200">
        <h1 className="text-2xl font-bold text-center text-gray-900 mb-6">{t("auth", "signIn")}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              ...
              <Link href="/forgot-password" className="text-xs text-indigo-600 hover:underline">{t("auth", "forgotPassword")}</Link>
            </div>
          ...
          <button type="submit" disabled={loading} className="w-full bg-indigo-600 text-white py-2 rounded-md font-medium hover:bg-indigo-700 disabled:opacity-50">
            {loading ? "Signing in..." : t("auth", "signInBtn")}
          </button>
        </form>
        <p className="text-sm text-center text-gray-600 mt-4">
          {t("auth", "noAccount")}{" "}
          <Link href="/register" className="text-indigo-600 hover:underline">{t("auth", "signUp")}</Link>
        </p>
      </div>
    </div>
  );
